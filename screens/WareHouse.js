...
import {
  View, Text, StyleSheet, FlatList, Dimensions, TouchableOpacity, Alert, Image, TextInput,
} from "react-native";
...
const Warehouse = () => {
  ...
  const handleSearch = (text) => {
    const filtered = warehouses.filter((warehouse) =>
      warehouse.name.toLowerCase().includes(text.toLowerCase())
    );
...
const styles = StyleSheet.create({
  ...
  warehouseCard: {
    ...
    shadowRadius: 4,
    elevation: 3,
    width: '95%', // Adjusted width to 90% of the container
    alignSelf: 'center',
  },
  textContainer: {
    flex: 1,
